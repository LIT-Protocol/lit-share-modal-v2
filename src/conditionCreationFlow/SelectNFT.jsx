...
} from "../helpers/conditionGenerator";
import { litJsSdkLoadedInALIT } from "lit-js-sdk";
...
const SelectNFT = ({ setSelectPage, handleUpdateAccessControlConditions }) => {
  ...
  const handleSubmit = () => {
    const accessControlConditions = generateUnifiedCondition({
      chain: chain.value,
      conditionType: CONDITION_TYPES.HOLDS_SPECIFIC_NFT,
      params: { contractAddress: selectedToken?.value, tokenId },
    });
  ...
  return (
    <div className={"lsm-select-container"}>
      ...
      />
      {!chain ||
      (chain && LitJsSdk.ALL_LIT_CHAINS[chain.value]?.vmType === "EVM") ? (
      ...
      <div className={"lsm-w-full"}>
        ...
        <LitInput value={tokenId} setValue={setTokenId} />
      </div>
      <p
        className={
          "lsm-text-sm md:lsm-text-base lsm-underline  lsm-w-full lsm-mt-8 lsm-cursor-pointer lsm-mb-4 lsm-text-title-gray lsm-text-left lsm-font-segoe lsm-font-light"
        }
      ...
      <LitFooter
        ...
        nextDisableConditions={
          !chain ||
          !tokenId.length ||
          // only require selectedToken for EVM chains
          (chain &&
